...
namespace QuickFix.Views
{
    ...
    public partial class PlumberMap : ContentPage
    {
        ...
        public async void GetAllPlumber()
        {
            bool gpsStatus = DependencyService.Get<ILocSettings>().isGpsAvailable();
            if (gpsStatus == false)
            {
            ...
            var response = await _techService.GetAllPlumber();
            if (response != null)
            {
                ...
                foreach (var item in response)
                {
                    ...
                    if (distance <= 5)
                    {
                        ...
                        lst.Add(new TechnicianDetails() { Id = item.TID, Name = item.FullName, Latitude = (double)item.Latitude, Longitude = (double)item.Longitude, Gender = item.Gender, Phone = item.PhoneNumber, Address = item.Address, DisatanceAway = (float)distance, Technician = "Plumber" });

                    }
                }
                if (lst.Count == 0)
                {
                    bool result = await DisplayAlert("Not Found", "No Plumber Found In Your Locality Go For Further??", "OK", "Cancel");
                    if (result)
                    {
                        foreach (var item in response)
                        {
